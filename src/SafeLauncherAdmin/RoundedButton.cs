...
internal sealed class RoundedButton : Button
{
    ...
    protected override void OnPaint(PaintEventArgs pevent)
    {
        ...
        // Fill entire control with parent background to handle rounded corners
        ...
        Color fill = Enabled
            ? isPressed ? PressedBackColor : isHovered ? HoverBackColor : BackColor
            : Color.FromArgb(226, 232, 240);
        Color text = Enabled ? ForeColor : Color.FromArgb(148, 163, 184);

        using SolidBrush fillBrush = new(fill);
        using Pen borderPen = new(Enabled ? BorderColor : Color.FromArgb(203, 213, 225), 1f);
